...
def parse_text_column(text):
    ...
    # Extract the question number (first line)
    question_number = lines[0]
    
    # Find the question text (lines ending with a '?' or part of the main question block)
    question_lines = []
    i = 1
    ...
    # Extract four valid answers
    answers = []
    answer_pattern = re.compile(r'^(?:[a-dA-D]\s|\d\s|[A-D]\)|.*\s[a-zA-Z]+)')  # Match typical answer-like structures
    while i < len(lines):
        if len(answers) >= 4:  # Stop after collecting 4 answers
            break
        if lines[i].strip():  # Check if the line matches an answer pattern
            answers.append(lines[i])
        i += 1

    # Ensure exactly four answers are present
    if len(answers) != 4:
        raise ValueError(f"Error: Expected 4 answers, but found {len(answers)} in text:\n{text}")
    
    # Trim any extra characters from answers (optional)
    answers = [re.sub(r'[^\w\s]', '', answer) for answer in answers]
